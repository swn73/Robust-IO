//
// Created by root on 15-4-21.
//

#ifndef TEST_PROJECT_RIO_H
#define TEST_PROJECT_RIO_H
...
ssize_t rio_readn(int fd, void *usrbuf, size_t n) {
    ...
    while (nleft > 0) {
        if ((nread = read(fd, bufp, nleft)) < 0) {
            if (errno == EINTR) {   //  will call read() again.
...
ssize_t rio_writenb(rio_t *p, void *usrbuf, size_t n);
